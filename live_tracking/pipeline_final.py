...
class Experiment:
    ...
    def reset_call(self):
        '''
        Function to set the reset variable to true when using the GUI in order to reset the fly and magnet IDs
        '''
        self.reset = True    
    ...
    def switch_command(self):
        '''
        Function useful for GUI to switch the IDs of the fly and the magnet if they have been incorrectly initialised
        '''
        if self.robotID == 0:
            self.robotID = 1
            self.fly_id = 0
    ...
    def process_video(self, frame, count, pca_reloaded, modelUp_or_Down, model_CNN, cnn, svm, both, t_id = ['A', 'B'], colors = [(0,0,255),(0,255,255)], n_inds = 2):
        
        total_start_time = time.time()

        #initialise the needed variables
        frame_color_output = cv.cvtColor(frame.copy(), cv.COLOR_GRAY2BGR)
        frame_gray_analysis = frame.copy()
        radius_robot_center = 0
        start_point_x = 0
        end_point_x = 0
        start_point_y = 0
        end_point_y = 0
        ...
        correction_elapsed_time = 0
        detection_elapsed_time = 0
        reorder_elapsed_time = 0
        fly_head_elapsed_time = 0
        facing_elapsed_time = 0
        feasible_pos_elapsed_time = 0
        draw_magnet_elapsed_time = 0
        ...
        if count >= 10:
            ...
            preprocess_elapsed_time = time.time() - preprocess_time
            ...
            if len(contours) < n_inds:
                ...
                print('meas_last of frame {} after correction is {}'.format(count, self.meas_last))
                print('meas_now of frame {} after correction is {}'.format(count, self.meas_now))
                correction_elapsed_time = time.time() - correction_time
            else:
            ...
            if len(self.meas_now) == n_inds and len(contours) >= 2:
                ...
                contours, self.meas_now = reorder_and_draw(frame_color_output, colors, n_inds, col_ind, contours, count, self.meas_now)
                if count > 20 and len(contours[self.fly_id]) < 5:
                    print('here')
                    right_number_contours = False
                reorder_elapsed_time = time.time() - reorder_time

            if right_number_contours == True:
                # Determine which is the fly and which is the magnet
                if count == 10 or self.reset == True:
                    #self.fly_id = determine_fly_robot(self.meas_now)
                    self.fly_id = determine_fly_robot2(contours)
                    self.robotID = (self.fly_id + 1) % 2
                    self.robot_size = find_robot_size(contours[self.robotID])
                    self.reset = False

                # Find the fly's head position
                fly_head_time = time.time()
                contour_fly = contours[self.fly_id]
                angle, translated_rotated_mask1, translated_rotated_mask2, rotated_mask, rotated_frame, center, fly_size = self.get_fly_orientation(frame_color_output, frame_no_walls, contour_fly)
                fly_head_elapsed_time = time.time() - fly_head_time

                # Find if the fly is facing up or down
                if (count % 10) == 0:
                    facing_time = time.time()
                    ...
                    self.predicted_way = predict_facing(cnn, svm, both, pca_reloaded, modelUp_or_Down, model_CNN, final_ML)
                    ...
                    facing_elapsed_time = time.time() - facing_time

                if self.predicted_way == 1:
                ...
                fly_angle = angle
                fly_facing = self.predicted_way

                # Find feasible robot positions      
                feasible_pos_time = time.time()          
                vertical_walls, horizontal_walls = self.find_vertical_horizontal_walls(self.meas_now[self.fly_id])
                feasible_angle, radius_robot_center = feasible_angles(vertical_walls, horizontal_walls, fly_size,self.robot_size, 40, 5, self.meas_now[self.fly_id], frame_color_output)
                start_point_x, start_point_y, end_point_x, end_point_y, start_ang_h, finish_ang_h = define_StartEnd_points(feasible_angle, self.meas_now[self.fly_id], radius_robot_center)
                feasible_pos_elapsed_time = time.time() - feasible_pos_time
                
                #determine if it is possible to do the desired trajectory with the desired start and end angles
                desired_start = 90
                desired_end = 180
                possible = determine_if_possible(desired_start, desired_end, feasible_angle, fly_angle)

                #draw extreme positions of the magnet:
                draw_magnet_time = time.time() 
                draw_end_pos(start_point_x, start_point_y, end_point_x, end_point_y, self.meas_now, self.fly_id, self.robot_size, frame_color_output)
                draw_magnet_elapsed_time = time.time() - draw_magnet_time

                # Create output dataframe
                for i in range(n_inds):
                    new_data = [[count, self.meas_now[i][0], self.meas_now[i][1], t_id[i], fly_angle, fly_facing]]
                    new_rows = pd.DataFrame(new_data)
                    self.df = pd.concat([self.df, new_rows], ignore_index=True)

                self.KMeansProcessedFrames = self.kMeansFrames

                delta_x_robot =  self.meas_now[self.robotID][0] - self.meas_now[self.fly_id][0]
                delta_y_robot = self.meas_now[self.robotID][1] - self.meas_now[self.fly_id][1]

            #convert angles to trigonometric angles
            if start_ang_h < finish_ang_h:
                save_finish = finish_ang_h
                finish_ang_h = start_ang_h + 360
                start_ang_h = save_finish
            else:
                save_finish = finish_ang_h
                finish_ang_h = start_ang_h
                start_ang_h = save_finish
            
            total_elapsed_time = time.time() - total_start_time

            # Store metrics
            self.metrics.append((count, time.time(), total_elapsed_time, preprocess_elapsed_time, correction_elapsed_time, detection_elapsed_time, reorder_elapsed_time, fly_head_elapsed_time, facing_elapsed_time, feasible_pos_elapsed_time, draw_magnet_elapsed_time))
            
        return frame_color_output, radius_robot_center, delta_x_robot, delta_y_robot, start_ang_h, finish_ang_h, self.meas_now[self.fly_id]
