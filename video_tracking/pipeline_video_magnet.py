...
class Experiment:
    ...
    def process_video(self, pca_reloaded, modelUp_or_Down, model_CNN, cnn, svm, both):
        ...
        while(True):
            ...
            ret, frame = cap.read()
            if ret == True and c > 400 and c < 500:
                resized_frame = cv.resize(frame, (1024, 1024))
                raw_imgs.append(resized_frame)
            elif c>500:
                break
            c+=1
        ...
        for frame_number, frame in enumerate(raw_imgs):
            ...
            if frame_number >= 2:
                ...
                if right_number_contours == True:
                    ...
                    if (frame_number == 2 or set_fly == False) :
                    ...
                    for i in range(n_inds):
                        new_data = [[frame_number, self.meas_now[i][0], self.meas_now[i][1], t_id[i], fly_angle, fly_facing]]
            ...
            self.metrics.append((frame_number, time.time(), total_elapsed_time, preprocess_elapsed_time, correction_elapsed_time, detection_elapsed_time, reorder_elapsed_time, fly_head_elapsed_time, facing_elapsed_time, feasible_pos_elapsed_time, draw_magnet_elapsed_time))
