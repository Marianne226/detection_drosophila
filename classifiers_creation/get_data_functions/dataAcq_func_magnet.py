...
def determine_fly_robot2(contours):
    """
    Determines the index of the contour with the largest area, assumed to be the fly.

    Parameters
    ----------
    contours : list of ndarray
        A list of contours detected in the frame.

    Returns
    -------
    fly_id : int
        The index of the contour with the largest area.
    """
    
    previous_area = 0
    fly_id = -1  # Initialize fly_id to an invalid index

    for i, contour in enumerate(contours):
        print(len(contours))  # Debugging statement to print the number of contours
        area = cv.contourArea(contour)
        
        # Update fly_id if the current contour's area is larger than the previous largest area
